...
def pad_history_array(histories, key, target_len=None, fill_value=0.0):
    """Convert a list of variable-length history dicts into a 2D array.

    Rows = independent stochastic runs.
    Columns = days.

    Runs that stop early because the colony went extinct are padded with zero,
    which is the correct value for population counts after extinction.
    """
    if target_len is None:
        target_len = max(len(h[key]) for h in histories)

    arr = np.full((len(histories), target_len), fill_value, dtype=float)
...
def ensemble_stats(histories, key, target_len=None):
    """Return day-by-day mean, variance, std, and standard error for one variable."""
    data = pad_history_array(histories, key, target_len=target_len)
    n_runs = data.shape[0]
    mean = np.mean(data, axis=0)
    var = np.var(data, axis=0, ddof=1) if n_runs > 1 else np.zeros(data.shape[1])
    std = np.sqrt(var)
    sem = std / np.sqrt(n_runs) if n_runs > 0 else std
    return {
        "data": data,
        "mean": mean,
        "var": var,
...
def extinction_days(histories, target_len=None):
    """Return first extinction day for each run; NaN means it never went extinct."""
    alive = pad_history_array(histories, "n_alive", target_len=target_len)
    days = []
    for row in alive:
        zero_idx = np.where(row <= 0)[0]
        days.append(float(zero_idx[0]) if len(zero_idx) > 0 else np.nan)
    return np.array(days, dtype=float)


def save_metric_table(histories, output_dir, filename="06_stochastic_metrics.csv", days_to_report=(200, 250, 300, 365, 500, 730)):
    """Save mean/variance/std metrics at selected days for the writeup."""
    target_len = max(len(h["day"]) for h in histories)
    keys = ["n_alive", "n_infected", "spore_conc", "food"]

    rows = []
    for day in days_to_report:
        # If day 730 is requested but the simulation only recorded 0...729,
        # use the final available day instead of crashing.
        idx = min(day, target_len - 1)
        for key in keys:
            stats = ensemble_stats(histories, key, target_len=target_len)
            rows.append({
                "requested_day": day,
                "actual_day_index_used": idx,
                "variable": key,
                "mean": stats["mean"][idx],
                "variance": stats["var"][idx],
                "std_dev": stats["std"][idx],
                "standard_error": stats["sem"][idx],
            })

    ext = extinction_days(histories, target_len=target_len)
    survival_prob = np.mean(pad_history_array(histories, "n_alive", target_len=target_len)[:, -1] > 0)
    extinction_prob = 1.0 - survival_prob
    mean_extinction_day = np.nanmean(ext) if np.any(~np.isnan(ext)) else np.nan

    summary_rows = [
        {"metric": "n_runs", "value": len(histories)},
        {"metric": "survival_probability_final_day", "value": survival_prob},
        {"metric": "extinction_probability_final_day", "value": extinction_prob},
        {"metric": "mean_extinction_day_conditional_on_extinction", "value": mean_extinction_day},
    ]

    csv_path = os.path.join(output_dir, filename)
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["requested_day", "actual_day_index_used", "variable", "mean", "variance", "std_dev", "standard_error"])
        writer.writeheader()
        writer.writerows(rows)

    summary_path = os.path.join(output_dir, filename.replace(".csv", "_summary.csv"))
    with open(summary_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["metric", "value"])
        writer.writeheader()
        writer.writerows(summary_rows)

    print(f"  Saved day-by-day metric table to: {csv_path}")
    print(f"  Saved summary metric table to: {summary_path}")
    print(f"  Final survival probability = {survival_prob:.3f}")
    print(f"  Final extinction probability = {extinction_prob:.3f}")
    if not np.isnan(mean_extinction_day):
        print(f"  Mean extinction day, among extinct runs = {mean_extinction_day:.1f}")

    # Also print the most useful values for the report text.
    for day in days_to_report:
        idx = min(day, target_len - 1)
        alive_stats = ensemble_stats(histories, "n_alive", target_len=target_len)
        inf_stats = ensemble_stats(histories, "n_infected", target_len=target_len)
        print(
            f"  Day {day} (index {idx}): "
            f"alive mean={alive_stats['mean'][idx]:.2f}, alive var={alive_stats['var'][idx]:.2f}; "
            f"infected mean={inf_stats['mean'][idx]:.2f}, infected var={inf_stats['var'][idx]:.2f}"
        )
...
def rmse_against_deterministic(stochastic_histories, deterministic_history, stochastic_key, deterministic_key=None, start_day=0, end_day=None):
    """Compare stochastic ensemble mean against a deterministic curve.

    Example usage after you create an ODE history:
        rmse_alive = rmse_against_deterministic(histories, det, "n_alive")
        rmse_infected = rmse_against_deterministic(histories, det, "n_infected")
        rmse_spores = rmse_against_deterministic(histories, det, "spore_conc")
    """
    if deterministic_key is None:
        deterministic_key = stochastic_key

    target_len = max(len(h["day"]) for h in stochastic_histories)
    stoch_mean = ensemble_stats(stochastic_histories, stochastic_key, target_len=target_len)["mean"]
    det = np.asarray(deterministic_history[deterministic_key], dtype=float)

    m = min(len(stoch_mean), len(det))
    if end_day is None:
        end_day = m - 1
    mask = np.arange(m)
    mask = (mask >= start_day) & (mask <= end_day)

    return float(np.sqrt(np.mean((stoch_mean[:m][mask] - det[:m][mask]) ** 2)))
...
def plot_stochastic_vs_mean(histories, save_path=None, metrics_output_dir=None):
    """Plot multiple stochastic runs, their mean, and ±1 std. Also save metrics."""
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 8), sharex=True)

    max_len = max(len(h["day"]) for h in histories)
    days = np.arange(max_len)

    alive_stats = ensemble_stats(histories, "n_alive", target_len=max_len)
    inf_stats = ensemble_stats(histories, "n_infected", target_len=max_len)

    mean_alive = alive_stats["mean"]
    std_alive = alive_stats["std"]
    mean_infected = inf_stats["mean"]
    std_infected = inf_stats["std"]

    # individual runs
    for h in histories:
        ax1.plot(h["day"], h["n_alive"], color="tab:blue", alpha=0.08, linewidth=0.5)
        ax2.plot(h["day"], h["n_infected"], color="tab:red", alpha=0.08, linewidth=0.5)

    # mean ± 1 standard deviation band
    ax1.fill_between(days, mean_alive - std_alive, mean_alive + std_alive,
                     color="tab:blue", alpha=0.20, label="±1 std. dev.")
    ...
    ax2.fill_between(days, mean_infected - std_infected, mean_infected + std_infected,
                     color="tab:red", alpha=0.20, label="±1 std. dev.")
    ...
    if metrics_output_dir is not None:
        save_metric_table(histories, metrics_output_dir)

    return {
        "mean_alive": mean_alive,
        "var_alive": alive_stats["var"],
        "std_alive": std_alive,
        "mean_infected": mean_infected,
        "var_infected": inf_stats["var"],
...
#    run_immune_sweep(output_dir)
    ...
    run_stochastic_variability(output_dir)  # uncomment to generate mean/variance metrics CSV
